...
- **THEN** 「{国名}：米を売却し、金{amount}を得ました」というメッセージとともに `Domestic / Player` ログが保存される

#### Scenario: log-develop-land
- **WHEN** プレイヤーが開墾コマンドを実行したとき
- **THEN** 「{国名}：開墾し、石高が{gain}上昇しました」というメッセージとともに `Domestic / Player` ログが保存される

#### Scenario: log-build-town
- **WHEN** プレイヤーが町作りコマンドを実行したとき
- **THEN** 「{国名}：町を整備し、町が{gain}上昇しました」というメッセージとともに `Domestic / Player` ログが保存される

#### Scenario: log-recruit-troops
- **WHEN** プレイヤーが徴募コマンドを実行したとき
- **THEN** 「{国名}：兵を{amount}徴募しました」というメッセージとともに `Domestic / Player` ログが保存される
...
- **THEN** 「{from国名}→{to国名}：資源を輸送しました（金:{kin} 兵:{hei} 米:{kome}）」というメッセージとともに `Domestic / Player` ログが保存される
...
- **THEN** 「自軍({自軍戦術})の被害: {x}、敵軍({敵軍戦術})の被害: {y}」という `War / Player` ログが保存される
