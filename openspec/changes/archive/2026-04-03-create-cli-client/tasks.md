...
- [x] 4.3 コマンド決定時の数値入力および対象国選択のフロー（SubState）を実装する
- [x] 4.4 `DomesticUseCase` を呼び出し、各種コマンドのロジック実行と再描画を連携させる

## 5. War Mode UI and Logic

- [x] 5.1 戦争対象の領地を選択するUIを実装する
- [x] 5.2 戦争モード時の両軍ステータスと、戦術選択（通常、奇襲、火計など）メニューを描画するパネルを実装する
- [x] 5.3 `BattleUseCase` を呼び出し、戦闘の実行結果を描画・反映する処理を実装する

## 6. Turn Progression and Messaging

- [x] 6.1 `TurnProgressionUseCase` と連携し、プレイヤー行動終了時やNPCターンの処理をUI上から実行させる
- [x] 6.2 ターン進行中やコマンド結果のメッセージ表示（ShowMessageサブステート）を実装する
