...
mod tests {
    ...
    use crate::domain::model::value_objects::{DaimyoId, KuniId, IninFlag, TurnNumber, ActionOrderIndex, DisplayAmount, INTERNAL_SCALE};
    ...
    use crate::domain::model::action_log::{ActionLogEntry, ActionLogCategory};
    ...
    impl KuniRepository for MockKuniRepository {
        ...
        async fn find_by_daimyo_id(&self, daimyo_id: &DaimyoId) -> Result<Vec<Kuni>, DomainError> {
            let mut kunis: Vec<_> = self.kunis.read().await.values().filter(|k| &k.daimyo_id == daimyo_id).cloned().collect();
    ...
    impl EventDispatcher for MockEventDispatcher {
        async fn dispatch(&self, _event: GameEvent) -> Result<(), DomainError> { Ok(()) }
    }
    ...
    // --- Helper ---
    fn to_internal(val: u32) -> u32 { val * INTERNAL_SCALE }

    // --- Tests ---

    #[tokio::test]
    async fn test_get_other_countries_info() {
        let kuni_repo = Arc::new(MockKuniRepository { kunis: RwLock::new(HashMap::new()) });
        let daimyo_repo = Arc::new(MockDaimyoRepository { daimyos: RwLock::new(HashMap::new()) });
        let game_state_repo = Arc::new(MockGameStateRepository { state: RwLock::new(None) });
        ...
        kuni_repo.save(&Kuni::new(
            player_kuni_id, "プレイヤー国", player_id,
            Resource::new(to_internal(100), to_internal(10), to_internal(100), to_internal(500)),
        ...
        kuni_repo.save(&Kuni::new(
            enemy_kuni_id, "敵国", enemy_id,
            Resource::new(to_internal(1000), to_internal(500), to_internal(1000), to_internal(2000)),
